...
class Invoice extends CI_Controller {
    ...
    public function view() {
        ...
        $data['customers'] = $this->invoice_model->get_all_customers();
        $this->load->view('include/header');
        $this->load->view('admin/invoice/view_invoice', $data);
        $this->load->view('include/footer');
    }
    ...
    public function add_edit()
    {
        ...
        $redirectTo = base_url().'admin/invoice/view';
        ...
        if($this->input->post('id') && $this->input->post('id') != ''){
            if($this->input->post('view') && $this->input->post('view') == 'view') {
                $redirectTo = base_url().'admin/invoice/edit/'.$this->input->post('id');
                $this->session->set_userdata('admin/invoice/view_invoice', $file);
            } else {
            ...
            $redirectTo = base_url().'admin/invoice/edit/'.$res;
            $this->session->set_userdata('admin/invoice/view_invoice', $file);
        }
        $this->invoice_model->updateRow('admin/invoice/invoice', 'invoice_id', $res, $inst_arr);
    ...
    public function generatePdf($id){
        ...
        $body = $this->load->view('admin/invoice/templates/invoice', '', true);
        $sett = $this->invoice_model->get_data_by('invoice_settings', 1, 'id');
        ...
        $table = '<table class="table m-t-30">
                    ...
                    $table .= '<tr class="even">
                                    ...
                                    <td colspan="4">'.$invoice_detail->subtotal.'</td>
                                </tr>';
    ...
    public function setting() {
        ...
        if($id && $id > 0) {
            ...
            $art_msg['type'] = 'success';
            $this->session->set_userdata('alert_msg', $art_msg);
            redirect(base_url().'admin/invoice/setting');
        } else {
            ...
            $this->load->view('admin/invoice/inv_setting');
            $this->load->view('include/footer');
        }
    }
    ...
    public function pdf_path($id) {
        ...
        if(isset($invoice_detail->file_name) && $invoice_detail->file_name != '') {
            if(isset($this->session->get_userdata()['admin/invoice/view_invoice'])) {
                $this->session->unset_userdata('admin/invoice/view_invoice');
            }
    ...
    public function calculateTax($amt, $dis) {
        ...
        foreach ($taxes as $taxkey => $taxvalue) {
            if($taxvalue->calculate_on == 'sub_total') {
                $amount = $this->calculatePersentage($taxvalue->tax_value, $total_tax);
