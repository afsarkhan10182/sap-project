...
class User extends CI_Controller {
    ...
    public function index() {
            if(is_login()){
    		redirect( base_url().'admin/user/dashboard', 'refresh');
    	}
    }
    ...
    public function dashboard() {
		...
		$qr = " SELECT count(*) AS `mka` FROM `users`   ";
		$data['TOTAL_USERS_data'] = $this->User_model->getQrResult($qr);

		$qr = " SELECT count(*) AS `mka` FROM `users`   WHERE  `users`.`status` = 'active' ";
		$data['ACTIVE_USERS_data'] = $this->User_model->getQrResult($qr);

		$qr = " SELECT count(*) AS `mka` FROM `users`   WHERE  DATE_FORMAT(`users`.`create_date`, '%Y-%m-%d') = '".date('Y-m-d')."'  ";
		$data['_TODAY_REGISTERED_data'] = $this->User_model->getQrResult($qr);
		/* mka_info_dashboard */


					/* list Box function */
					$query = "SELECT MONTH(`create_date`) as `months`, '1' AS `mka_sum` FROM  `users`  WHERE YEAR(`create_date`) = '2017'  ";
					$data['Registraion_in_months'] = $this->User_model->get_bar_chart_data($query);
			$query = "SELECT * FROM  `users`   ORDER BY `users_id` DESC LIMIT 10 ";
        ...
        if($this->input->post()){
            ...
            if(isset($res[0]->users_id) && $res[0]->users_id != '') {
                ...
                $data = array(
                    ...
                    'varification_link' => base_url().'admin/user/mail_varify?code='.$var_key,
                    'url_link' => base_url().'admin/user/mail_varify?code='.$var_key,
                    );
                ...
                $art_msg['type'] = 'danger';
                $this->session->set_userdata('alert_msg', $art_msg);
                redirect( base_url()."admin/user/forgetpassword");
            }
        } else {
            $this->load->view('admin/user/forget_password');
        }
    }
    ...
    public function userTable(){
        ...
        if(CheckPermission("user", "own_read") || CheckPermission("user", "all_read")){
            $this->load->view('include/header');
            $this->load->view('admin/user/user_table');
            $this->load->view('include/footer');
        } else {
    ...
    public function add_edit($id='') {
        ...
        if($id != '') {
            ...
            if($checkValue==false)  {
                ...
                $art_msg['type'] = 'info';
                $this->session->set_userdata('alert_msg', $art_msg);
                redirect( base_url().'admin/user/userTable', 'refresh');
            }
                ...
                if($this->input->post('currentpassword') != '') {
                    ...
                    if(password_verify($this->input->post('currentpassword'), $old_row->password)){
                        if($this->input->post('password') == $this->input->post('confirmPassword')){
                            ...
                            $art_msg['type'] = 'warning';
                            $this->session->set_userdata('alert_msg', $art_msg);
                            redirect( base_url().'admin/user/'.$redirect, 'refresh');
                        }
                    } else {
            ...
            if($this->input->post('user_type') != 'admin') {
                ...
                $art_msg['type'] = 'danger';
                $this->session->set_userdata('alert_msg', $art_msg);
                redirect( base_url().'admin/user/registration', 'refresh');
            }
        }
    ...
    public function registerMail($data1){
        ...
        if(isset($res[0]->users_id) && $res[0]->users_id != '') {
            ...
            $data = array(
                'user_name' => $res[0]->name,
                'varification_link' => base_url().'admin/user/registration_mail_varify?code='.$data1['var_key'],
                'website_name' => $setting['website'],
                );
	    		...
	    		foreach ($emailArray as $mailKey => $mailValue) {
	    			if(filter_var($mailValue, FILTER_VALIDATE_EMAIL)) {
	    				...
	    				if(is_array($res) && empty($res)) {
			    			$link = (string) '<a href="'.base_url().'admin/user/registration?invited='.$var_key.'">Click here</a>';
