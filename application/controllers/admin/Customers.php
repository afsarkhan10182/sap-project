...
class Customers extends CI_Controller {
  	function __construct() {
	    ...
	    $this->lang->load(strtolower('Customers'), 'english');
		if(true==1){
			is_login();
      ...
      */
  	public function index() {
        if(CheckPermission("customers", "all_read,own_read")){
            $con = '';
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Customers_model->Get_data_id($this->input->post('id'));
      		echo $this->load->view('admin/customers/add_update', $data, true);
	    } else {
	      	echo $this->load->view('admin/customers/add_update', '', true);
	    }
	    exit;
	}
...
array( 'db' => 'customers.name AS name', 'field' => 'name', 'dt' => 1 ),
		...
		$recordsTotal = $this->db->select("count('customers') AS c")->get('customers')->row()->c;
