...
class Products extends CI_Controller {
  	function __construct() {
	    ...
	    $this->lang->load(strtolower('Products'), 'english');
		if(true==1){
			is_login();
      ...
      */
  	public function index() {
  		if(CheckPermission("products", "all_read,own_read")){
  			$con = '';
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Products_model->Get_data_id($this->input->post('id'));
      		echo $this->load->view('admin/products/add_update', $data, true);
	    } else {
	      	echo $this->load->view('admin/products/add_update', '', true);
	    }
	    exit;
	}
...
array( 'db' => 'products.unit_price AS unit_price', 'field' => 'unit_price', 'dt' => 3 ),
		...
		$recordsTotal = $this->db->select("count('products') AS c")->get('products')->row()->c;
