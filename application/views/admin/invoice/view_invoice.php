        ...
        <div class="card">
          <div class="header">
            ...
            <ul class="header-dropdown">
             ...
             <a href="<?php echo base_url().'admin/invoice'; ?>" class="btn-sm  btn btn-primary waves-effect amDisable modalButton"><i class="material-icons">add</i> <?php echo lang('add_invoice'); ?></a><?php } ?>
              </ul>
...
<script>
  $(document).ready(function() {
    $('body').on('click', '.generateInv', function() {
      $.ajax({
        url: '<?php echo base_url().'admin/invoice/pdf_path/' ?>'+$(this).attr('rel'),
        method:'post',
      }).done(function(data){
        if(data != 'no_img') {
...
function loadDatatable(date, status) {
  ...
  $ret = $("#example1").DataTable({
        ...
        "serverSide": true,
        "ajax": {
          "url" : '<?php echo base_url(); ?>' + "admin/invoice/dataTable",
          "data": function ( d ) {
