						...
						<div class="header">
                           <h2><?php echo lang('products') ?></h2>
                              <ul class="header-dropdown">
                                 <?php if(CheckPermission("products", "own_create")){ ?>
...
var table = $("#example_products").DataTable({
    		"dom": 'lfBrtip',
					  "buttons": ['copy','excel','pdf','print'],
        "processing": true,
        "serverSide": true,
        "ajax": {
        	"url" : url + "admin/products/ajx_data",
				...
				"columnDefs" : [
					{
						"orderable": false,
						"targets": [0]
					}<?php if(!CheckPermission("products", "all_delete") && !CheckPermission("products", "own_delete")){ ?>
					,{
            "targets": [0],
            "visible": false,
            "searchable": false
      ...
      table = $("#example_products").DataTable({
      	"dom": 'lfBrtip',
					  "buttons": ['copy','excel','pdf','print'],
        "processing": true,
        "serverSide": true,
      	"ajax": {
          "url" : url + "admin/ajx_data",
          "data": function ( d ) {
	...
	$('body').on('submit', '#form', function(ev) {
		...
		$.ajax({
			url: '<?php echo base_url().'admin/products/add_edit' ?>',
			method: 'POST',
			async: false,
			data: formData,
			cache: false,
...
$(document).ready(function(){
	...
	setTimeout(function() {
		...
		if(cjhk == 1) {
			$('.dataTables_info').before('<button data-del-url="<?php echo base_url() ?>products/delete/" rel="delSelTable" class="btn btn-default btn-sm delSelected pull-left"> <i class="material-icons col-red">delete</i> </button><br><br>');
		}
	}, 300);
	...
	$("body").on("click",".modalButton", function(e) {
		...
		$.ajax({
			url : "<?php echo base_url()."admin/products/get_modal";?>",
			method: "post",
			data : {
